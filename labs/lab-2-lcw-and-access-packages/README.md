...
- Make the user a member of a security group for ELNL 2026
- Make the user a member of a MS Teams group for ELNL 2026
...
- Create a custom extension with Logic App for 'privileged account requests' in the 'ELNL 2026' catalog
- Create an accesss package for 'privileged account requests' in the 'ELNL 2026' catalog.
...
- Next make sure to enable a 'system assigned managed identity' on the logic app created within this lab exercise.

Once all the above steps are completed proceed to the next lab exercise.
...
Since the logic app and custom extension have now been created but don't contain the low code steps, let's make sure to add these in. For that we have generated example source code which you can 'copy and paste' into the logic app which has been created. The source code for the logic app can be found at [LogicAppExample.json](../../resources/resource-2-custom-extension-example/CE-LogicAppExample.json).

Once the source code is copied into the Logic App, open the designer and make sure to change the following details:
- Edit the ObjectID in the first condition to match the of the **'ELNL 2026'** Catalog objectID.
- Edit the URI to reffer to your own Inbound Provisioning API endpoint within the step **'HTTP - Provision admin account'**, this is the same endpoint you've used in the previous lab exercise.
...
| Microsoft Entra | Authentication Administrator, Access Package Assignment Administrator (on the ELNL 2026 Catalog). | Required to generate the temporary access pass for a non-privileged user account and execute the API callback against the access package request in Entitlement Management |

For the Microsoft Graph permissions the following script can be used: [Configure-MsGRaphPermissions.ps1](../../resources/resource-3-msgraph-permissions/Configure-MsGraphPermissions.ps1)
...
Now the access package has been configured correctly lets verify if it's fully operational. This can be done by requesting the access package via [MyAccess](https://myaccess.microsoft.com) and make sure the entire process completes successfully and the callback of the custom extension is registered correctly in the access package request.

If this all succeeds the privileged account is created and enabled, the regular office account should have received an email with the Temporary Access Pass for onboarding with MFA. Now complete the MFA onboarding for this privileged account with the Temporary Access Pass, preferably with Authenticator app - push notification or other phishable credential like SMS or TOTP, as this account will be used in the 3rd next lab exercise.
...
1. What happens if you want to onboard a user in lifecycle workflows with a 'Generate TAP' step while the user already has sign-ins on their account?
2. Identify what the 'glue' is between the regular user object and the just created privileged account object (e.g. how do we know these belong together)?
3. What is the reasoning for using Exchange Online Plus addressing on admin accounts?
4. Should access packages being created as well to request individual roles or should is it better to use role-assignable groups within access pacakages to request additional privileges on the created privileged account?
5. How should the offboarding of privileged accounts be handeld, via lifecycle workflows or are there any other options?
...
- Is created within the 'ELNL 2026' catalog
...
Now the access package has been created successfully, let's make sure to add it to a lifecycle workflow which has been created earlier in this lab. We do this to make sure that for new joiners the access request is already put into the system and they don't have to request it theirselves in their first week of work.

For that edit the 'pre-hire' workflow and add 'request access to an access package' as a taks and select the previously created access package and make sure to **save** your edited workflow. Now run the workflow again on-demand and make sure that the access request is being made to the access package.
...
Once the access package is created, open the access package and add a new 'auto assignment policy' underneath policies. Scope the auto assignment policy to the department 'ELNL 2026'.
